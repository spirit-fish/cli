...
const syncFolderToStorage = async (bunny, storagezoneName, localFolder, remoteFolder) => {
  ...
  for (const path of paths) {
    await retry(async (bail, retry) => {
      try {
        await bunny.storage.update(`${storagezoneName}/${remoteFolder}/${path}`, fs.createReadStream(`${localFolder}/${path}`));
      } catch (e) {
        if (e && e.message === "Request body larger than maxBodyLength limit") {
          return bail({ status: 'error', reason: 'file.too_large', path });
        } else {
          throw e;
        }
      }
...
const aliasFile = async (bunny, from, to) => {
  await retry(async (bail, retry) => {
    const downstream = await bunny.storage.fetch(from, { responseType: 'stream' });
    await bunny.storage.update(to, downstream.data);
  }, {
    retries: 2,
    onRetry: Sentry.captureException
  });
...
const activateDeployment = async (bunny, storagezoneName, deploymentHash) => {
  ...
  for (const path of deploymentFilePaths) {
    ...
    await aliasFile(bunny, path, `${storagezoneName}${rawPath}`);
  }
